...
Given bialleic bubble collections
...
def get_svseq(source, norefbub, sink):
    ...
    try:
       totseq = grseq[source][-100:] if chainstrand[0] == "+" else revcomp(grseq[source])[-100:]
    except:
       totseq = grseq[source] if chainstrand[0] == "+" else revcomp(grseq[source])

    totseq += grseq[norefbub] if chainstrand[1] == "+" else revcomp(grseq[norefbub])

    try:
       totseq += grseq[sink][:100] if chainstrand[2] == "+" else revcomp(grseq[sink])[:100]

    except:
       totseq += grseq[sink] if chainstrand[2] == "+" else revcomp(grseq[sink])
