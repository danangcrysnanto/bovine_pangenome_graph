...
#graphlen  <- "graph/graph5_graph_len.tsv"
#nodemat  <- "analysis/colour_node/graph5_nodemat.tsv"

#' Given graph output the core and flexible genome size
#' @params graphlen: graphlen file in graph folder
...
pangenome_sampling <- function (graphlen, nodemat, graphtype, outbase =".") {
	...
	datpan  <- data.frame(norep=numeric(),
			      nosamp=numeric(),
                     	      assemb=character(),
			      core_gen=numeric(),
			      flex_gen=numeric(),
	...
	for (nosamp in seq(1, length(breeds))){
	    ...
	    datpan  <- rbind(datpan,data.frame(norep=norep,
					      nosamp=nosamp,
	...
	#add mean line
	datmean  <- datlong  %>% group_by(nosamp,pan_stat)  %>% summarise(mc=mean(val_stat))
	...
	pl1  <- ggplot(datlong,aes(x=nosamp,y=val_stat/10^6,group=nosamp))+
		...
		labs(x="Number of genome(s) in the pangenome",
			y="Size (Mb)")

	#save plot
	ggsave(pl1, filename=file.path(outbase,paste0(graphtype, "_core_flex_sim.pdf")),
	       width=10, height=12)

	ggsave(pl1, filename=file.path(outbase,paste0(graphtype, "_core_flex_sim.png")),
	       width=10, height=12)

	
	#save the sampling result
	write.table(datpan, file=file.path(outbase,paste0(graphtype, "_core_flex_sim.tsv")),
